...
const SectionMessage = ({ record, urgent }) => {
  ...
  const oldRequest = record.UpdateRequest
    ...
    : []
  const reqs = [...oldRequest, ...requestHistory]
  const displayedRequests = reqs.length > 3 ? reqs.slice(0, 3) : reqs

  return (
    <div>
      <h3 className="text-gsBlue font-semibold">Information Requests</h3>
      {urgent && (
        <button
      ...
      {displayedRequests.map(({ reqDate, reqMsg, respDate, respMsg, urgent: wasUrgent }, index) => (
        <div key={`${record.recordId}_request_${index}`}>
          {wasUrgent && <p>was urgent</p>}
          <RequestBlock msg={reqMsg} date={reqDate} />
      ...
      {expanded &&
        requestHistory.slice(3).map(({ reqDate, reqMsg, respDate, respMsg }, index) => (
